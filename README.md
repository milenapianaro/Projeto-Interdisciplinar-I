...
A logo do aplicativo G.U.I.A – Gestão Unificada de Informações do Agente – transmite cuidado e profissionalismo na área da saúde. À esquerda, um ícone em forma de coração com linha de batimentos, cruz médica e uma mão estilizada simboliza acolhimento e atenção ao bem-estar. As cores verde-azulado, amarelo e branco reforçam a ideia de confiança, vitalidade e clareza. Ao lado, o nome do sistema aparece em destaque, seguido da descrição que revela seu propósito: centralizar e organizar informações para agentes de saúde de forma eficiente e humanizada.
...
|------------------------|-----------------------------------------------------------------------|---------------|------------|------------------------------------------------------------------------------------|
| Tecnologia             | Falhas no sistema ou indisponibilidade da plataforma                 | Média         | Alto       | Monitoramento contínuo, redundância de servidores, plano de contingência          |
| Segurança da Informação| Vazamento de dados sensíveis de pacientes ou agentes de saúde        | Alta          | Muito alto | Criptografia de ponta a ponta, autenticação forte, auditorias regulares           |
| Legal e Regulatória    | Não conformidade com LGPD e normas da saúde pública                  | Média         | Alto       | Consultoria jurídica especializada, revisão periódica de políticas de privacidade |
| Operacional            | Resistência dos agentes à adoção da plataforma                       | Alta          | Médio      | Treinamentos, suporte técnico dedicado, comunicação clara sobre benefícios        |
| Financeiro             | Dificuldade de escalabilidade por falta de recursos                  | Média         | Alto       | Planejamento financeiro, busca por parcerias e editais públicos                   |
| Reputacional           | Percepção negativa por falhas iniciais ou problemas de usabilidade   | Média         | Alto       | Testes de usabilidade, canal de feedback ativo, melhorias contínuas               |
| Estratégico            | Concorrência com outras soluções de gestão de saúde                  | Alta          | Médio      | Diferenciação por funcionalidades, integração com sistemas públicos               |
| Governança             | Falta de clareza na gestão e tomada de decisão                       | Baixa         | Médio      | Estrutura de governança definida, com papéis e responsabilidades bem distribuídos |
...
_Link:_ https://www.canva.com/design/DAGzK8zLpnQ/Wv83KsZ-zZKjMH3oPZ5Zkg/edit
...
_Link (Figma):_ https://www.figma.com/proto/n9gjPc6w9RrYTWbxgc1Q7J/GUIA?t=q8LzyVarXPUjFwIS-0

## e) Roteiro de testes

Este plano cobre as principais funcionalidades do aplicativo, com foco em testes de caixa-preta para validar o comportamento observável do sistema.

__Autenticação__

| ID     | Caso de Teste           | Entrada                             | Saída Esperada                  |
|--------|--------------------------|-------------------------------------|----------------------------------|
| CT01   | Login válido             | Usuário válido + senha correta      | Tela inicial exibida            |
| CT02   | Login inválido           | Usuário válido + senha incorreta    | Mensagem de erro de autenticação|

__Registro de Visitas__

| ID     | Caso de Teste                   | Entrada                                      | Saída Esperada                          |
|--------|----------------------------------|----------------------------------------------|------------------------------------------|
| CT03   | Registrar visita sem coordenadas| Família selecionada + dados preenchidos      | Registro armazenado localmente           |
| CT04   | Registrar visita com coordenadas| Dados preenchidos + localização capturada    | Registro e coordenadas armazenados       |
| CT05   | Falha ao registrar visita       | Dados incompletos                            | Mensagem de erro + registro não salvo    |
| CT06   | Salvar rascunho                 | Dados parciais                               | Registro salvo como "rascunho"           |

__Sincronização e Persistência__

| ID     | Caso de Teste             | Entrada                          | Saída Esperada                         |
|--------|---------------------------|----------------------------------|-----------------------------------------|
| CT07   | Sincronizar dados         | Registros pendentes + conexão    | Status alterado para "sincronizado"     |
| CT08   | Falha de sincronização    | Conexão inativa                  | Mensagem de falha de sincronização      |
| CT09   | Operação offline          | Dados preenchidos + sem conexão | Registro salvo localmente como "pendente"|
| CT10   | Recuperar após reinício   | Registros locais existentes      | Dados mantidos após reinício do app     |
...
O desenvolvimento do aplicativo G.U.I.A será conduzido utilizando práticas do **Scrum**, com sprints curtos e entregas incrementais. Essa abordagem permitirá adaptação rápida às mudanças e acompanhamento contínuo da evolução do projeto. Elementos de **Kanban** também serão aplicados para o controle visual das tarefas e priorização.

## b) Recursos Utilizados
- **Tecnologias**: Aplicativo mobile híbrido (Flutter), backend em Node.js, banco de dados MySQL.  
- **Ferramentas de apoio**: GitHub para versionamento, Trello para gestão de tarefas, Figma para prototipação.  
- **Linguagem de programação**: Dart (frontend), JavaScript/TypeScript (backend).  
- **Equipamentos**: Computadores pessoais dos desenvolvedores, servidores de teste em nuvem.  
- **Rede**: Conexão estável à internet para sincronização e testes de integração.

## c) Resultados Esperados
- Garantir **centralização e unificação das informações** dos agentes de saúde.  
- Proporcionar **funcionalidade offline**, com sincronização posterior.  
- Melhorar a **eficiência operacional** e reduzir falhas no registro de visitas.  
- Oferecer uma interface intuitiva e acessível para os usuários finais.
...
_Link:_ https://drive.google.com/file/d/1ACvctgJ5pA8grGD7vTZK4M0ttRkjW0zH/view?usp=sharing

<br>

__Projeto 2:__

_Link:_ https://drive.google.com/file/d/17j9qdkMaZCKHT3Q6JzyzwjsvHgxClnA9/view?usp=drive_link

<br>

__Contexto sequenciador - MVP e incrementos:__ 
...
_Link:_ https://drive.google.com/file/d/16o4PNczwts5ltI5YO2mW01pQ9_wYxGr7/view?usp=sharing
